...
if not os.path.exists(nume_csv):
    ...
    print("Asigura-te ca ai rulat scriptul de extragere MediaPipe pe folderele cu abdomene!")
    exit()
...
joblib.dump(scaler, 'scaler_situps.pkl')
...
print("\nIncepem antrenarea pentru abdomene...")
istoric = model.fit(X_train, y_train,
                    epochs=150,
                    batch_size=32,
                    validation_data=(X_test, y_test))

model.save('model_situps.h5')
np.save('clase_situps.npy', label_encoder.classes_)

loss, accuracy = model.evaluate(X_test, y_test, verbose=0)
print(f"ACURATETEA FINALA PE TESTARE (SIT-UPS): {accuracy * 100:.2f}%")
...
plt.title('Evolutia Acuratetei Modelului de Abdomene')
