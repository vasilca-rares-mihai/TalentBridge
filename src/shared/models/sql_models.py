...
COMMON_ARGS = {'extend_existing': True, 'mysql_engine': 'InnoDB', 'mysql_charset': 'utf8mb4'}
...
class Trial(Base):
    ...
    id_trial = Column(BigInteger, primary_key=True, index=True, autoincrement=True)
    id_club = Column(BigInteger, ForeignKey('football_club.user_id', ondelete="CASCADE"), nullable=False, index=True)
...
class TrialApplications(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    id_trial = Column(BigInteger, ForeignKey('trial.id_trial', ondelete="CASCADE"), nullable=False, index=True)
    id_athlete = Column(BigInteger, ForeignKey('athlete.user_id', ondelete="CASCADE"), nullable=False, index=True)

    trial = relationship("Trial", back_populates="applications")
    athlete = relationship("Athlete", back_populates="applications")
...
class ChallengeResult(Base):
    ...
    user_id = Column(BigInteger, ForeignKey("athlete.user_id", ondelete="CASCADE"), nullable=False, index=True)
    challenge_id = Column(BigInteger, ForeignKey("challenge.id_challenge", ondelete="CASCADE"), nullable=False, index=True)
...
class Attribute(Base):
    ...
    user_id = Column(BigInteger, ForeignKey("athlete.user_id", ondelete="CASCADE"), primary_key=True, nullable=False)
...
class FavoriteAthlete(Base):
    ...
    # Corect: (UniqueConstraint, { dict_options })
    __table_args__ = (
        UniqueConstraint('club_id', 'athlete_id', name='_club_athlete_uc'),
        COMMON_ARGS
    )

    id = Column(BigInteger, primary_key=True, index=True, autoincrement=True)
    club_id = Column(BigInteger, ForeignKey('football_club.user_id', ondelete="CASCADE"), nullable=False)
    athlete_id = Column(BigInteger, ForeignKey('athlete.user_id', ondelete="CASCADE"), nullable=False)
